...
component displayname="SecurityService" accessors="true" extends="models.services.DatabaseSessionManagementService" {
	...
	* @return		string
	*/
	public string function dataEnc( required string value, string mode = 'db', string key = '' ) {
	...
	* @return		string
	*/
	public string function dataDec( required string value, string mode = 'db', string key = '' ) {
	...
	*/
	public string function getRandomPassword( numeric length = 0 ) {
		...
		if( structKeyExists( application, 'rejectHackedPasswords' ) && application.rejectHackedPasswords and isPasswordHacked( password ) ) {
	...
	* @description I return a MSSQL compatible GUID depending on the version of the engine in use
	* @param	   area {String} required - I am the area of the site to check if this user has access to
	* @param	   sessionObj {Any} required - I am the session object for the user to check
	* @return	   struct
	*/
	public struct function checkAccess( required string area, required any sessionObj, string event = '' ) {

		// check if the user belongs in the area
		if( findNoCase( arguments.area, arguments.sessionObj.getUserType() ) || ( findNoCase( 'administrator', arguments.sessionObj.getUserType() ) && findNoCase( 'admin', arguments.sessionObj.getUserRole() ) ) ) {
			// they do, return true
			return { canAccess = true };
		}

		// check if the user type is admin
		if( findNoCase( 'administrator', arguments.sessionObj.getUserType() ) ) {
			switch( lCase( arguments.sessionObj.getUserRole() ) ) {

				case 'photo':
					// check the user is in the 'photos' event
					if( findNoCase( 'photos', arguments.event ) ) {
						// they are, return true
						return { canAccess = true };
					}
					// otherwise, return false w/ a redirect to the admin login
					return { canAccess = false, redirect = 'admin.home.logout' };
				break;

				case 'biobook':
					// check the user is in the 'biobook' event
					if( findNoCase( 'biobook', arguments.event ) ) {
						// they are, return true
						return { canAccess = true };
					}
					// otherwise, return false w/ a redirect to the admin login
				...
				case 'report':
					// check the user is in the 'reports' event
					if( findNoCase( 'reports', arguments.event ) ) {
						// they are, return true
						return { canAccess = true };
					}
					// otherwise, return false w/ a redirect to the admin login
				...
				default:
					// if undefined role, logout and force login again (security)
					return { canAccess = false, redirect = 'admin.home.logout' };
				break;
		...
		// check if the user type is non existent
		if( findNoCase( 'nonexistentuser', arguments.sessionObj.getUserType() ) ) {
			// they are, return false w/ a redirect to the portal home page
			return { canAccess = false, redirect = 'main.index' };
		}

		// check if the user is a division rep
		if( findNoCase( 'representative', arguments.sessionObj.getUserType() ) ) {
			// they are, return false w/ a redirect to 'division'
			return { canAccess = false, redirect = 'division.home.logout' };
		}

		// otherwise, return false w/ a redirect to their user type (winner, executive, etc.)
		return { canAccess = false, redirect = arguments.sessionObj.getUserType() & '.home.logout' };
	...
	*/
	...
	/**
	* @displayname getHashIterationArray
	* @description I return an array of random has iteration values for use with uberHash/hash functions
	* @param	   arrayLength {Numeric} Default: 30 - I am the number of iteration values to put in the array
	* @param       lowestIteration {Numeric} Default: 25 - I am the lowest possible number of iterations
	* @param       highestIteration {Numeric} Default: 255 - I am the highest possible number of iterations
	* @return	   array
	*/
	public array function getHashIterationArray(
		numeric arrayLength = 30,
		numeric lowestIteration = 25,
		numeric highestIteration = 255
	) {

		var iterArr = [];

		for( var ix = 1; ix <= arguments.arrayLength; ix++ ) {
			iterArr.append( randRange( arguments.lowestIteration, arguments.highestIteration, 'SHA1PRNG' ) );
		}
		return iterArr;
	}
